...
namespace eKino.Desktop
{
    public partial class frmFilmDetalji : Form
    {
        ...
        {
            var film = _filmService.Get<List<Film>>(new FilmSearchRequest { Id = _filmId }).FirstOrDefault(); // _filmService.GetById<Film>(_filmId);
            ...
            if (k.Count == 1 && k[0].UlogaId == _ulogaService.Get<List<Uloga>>(new UlogaSearchRequest { NazivUloge = "Admin" }).Select(s => s.Id).FirstOrDefault()) //  _ulogaService.GetByName<Uloga>("Admin").Id
            {
            ...
            lblZanr.Text = _zanrService.Get<List<Zanr>>(new ZanrSearchRequest { Id = film.ZanrId }).Select(s => s.NazivZanra).FirstOrDefault(); //_zanrService.GetById<Zanr>(film.ZanrId).NazivZanra;
            lblTip.Text = _tipService.Get<List<Tip>>(new TipSearchRequest { Id = film.TipId }).Select(s => s.NazivTipa).FirstOrDefault(); //_tipService.GetById<Tip>(film.TipId).NazivTipa;
            ...
            {
                var prkuca = _prkucaService.Get<List<ProdukcijskaKuca>>(new ProdukcijskaKucaSearchRequest { Id = f.ProdukcijskaKucaId }).FirstOrDefault(); //_prkucaService.GetById<ProdukcijskaKuca>(f.ProdukcijskaKucaId);
            ...
            {
                var komentator = _korisnikService.Get<List<Korisnik>>(new KorisnikSearchRequest { Id = komentar.KomentatorId }).FirstOrDefault(); //_korisnikService.GetById<Korisnik>(komentar.KomentatorId);
        ...
        private void bttnDodajGlumca_Click(object sender, EventArgs e)
        {
            ...
            cbxGrad.Enabled = true;
            txtBiografija.Enabled = true;
        }
        ...
        private void OcistiTextBoxove()
        {
            ...
            tblDodavanjeGlumca.Visible = false;
            tblDodavanjeGlumca.Enabled = false;
            tblDodajPrKucu.Visible = false;
            tblDodajPrKucu.Enabled = false;
            tblDodajProjekciju.Visible = false;
            tblDodajProjekciju.Enabled = false;
            tblZaOcjenjivanjeFilma.Visible = false;
            tblZaOcjenjivanjeFilma.Enabled = false;
            tblZaDodavanjeKomentara.Visible = false;
            tblZaDodavanjeKomentara.Enabled = false;
        ...
        private void bttnDodajGlumcaDodaj_Click(object sender, EventArgs e)
        {
            if (cbxListGlumaca.SelectedIndex != 0)
            {
                ...
                var g = _glumacService.Get<List<Osoba>>(new OsobaSearchRequest { OsobaId = ((int)cbxListGlumaca.SelectedValue) }).FirstOrDefault(); //_glumacService.GetById<Osoba>((int)cbxListGlumaca.SelectedValue);
            ...
            else
            {
                if (this.ValidateChildren(ValidationConstraints.Enabled))
                {
                    var g = new OsobaInsertRequest
                    {
                        ...
                        UlogaId = _ulogaService.Get<List<Uloga>>(new UlogaSearchRequest { NazivUloge = "Glumac" }).Select(s => s.Id).FirstOrDefault() // ulogaService.GetByName<Uloga>("Glumac").Id
                    };
            ...
            else
            {
                if (this.ValidateChildren(ValidationConstraints.Enabled))
                {
                    var g = new OsobaInsertRequest
                    {
                        ...
                        DatumRodjenja = dtpDatum.Value,
                        GradId = (int)cbxGrad.SelectedValue,
                        UlogaId = _ulogaService.Get<List<Uloga>>(new UlogaSearchRequest { NazivUloge = "Scenarista" }).Select(s => s.Id).FirstOrDefault() // _ulogaService.GetByName<Uloga>("Scenarista").Id
                    };
                    var gl = _glumacService.Add<OsobaInsertRequest>(g);
        ...
        private void tblDodavanjeGlumca_VisibleChanged(object sender, EventArgs e)
        {
            if (tblDodavanjeGlumca.Visible == true)
            {
                if (lblNoviNaslov.Text.Contains("glumac"))
                {
                    ...
                    bttnDodajGlumcaDodaj.Click -= bttnDodajGlumcaDodaj_Click; // uklanja se jer poziva event 2 puta
                ...
                else if (lblNoviNaslov.Text.Contains("scenarista"))
                {
                    ...
                    cbxListGlumaca.ValueMember = "Id";
                    bttnDodajGlumcaDodaj.Click -= bttnDodajGlumcaDodaj_Click;
                    bttnDodajGlumcaDodaj.Click -= bttnDodajScenaristuDodaj_Click; // uklanja se jer poziva event 2 puta
                    bttnDodajGlumcaDodaj.Click += bttnDodajScenaristuDodaj_Click;
                }
        ...
        private void bttnDodajPrKucuDodaj_Click(object sender, EventArgs e)
        {
            if (cbxListKuca.SelectedIndex != 0)
            {
                ...
                var p = _prkucaService.Get<List<ProdukcijskaKuca>>(new ProdukcijskaKucaSearchRequest { Id = ((int)cbxListKuca.SelectedValue) }).FirstOrDefault(); // _prkucaService.GetById<ProdukcijskaKuca>((int)cbxListKuca.SelectedValue);
        ...
        private void bttnDodajPrKucu_Click(object sender, EventArgs e)
        {
            ...
            txtKucaNaziv.Enabled = true;
            cbxKucaGrad.Enabled = true;
        }
        ...
        private void bttnDodajProjekciju_Click(object sender, EventArgs e)
        {
            ...
            txtCijenaProjekcije.Enabled = true;
            txtOpisProjekcije.Enabled = true;
            cbxDvoranaProjekcije.Enabled = true;
        }
        ...
        private void bttnPotvrdiKomentar_Click(object sender, EventArgs e)
        {
            if (this.ValidateChildren(ValidationConstraints.Enabled))
            {
                ...
                OcistiTextBoxove();
                Task.Delay(1000);
                var k = _komentarService.Get<List<Komentar>>(new KomentarSearchRequest { KomentatorId = komentator.Id, SadrzajKomentara = komentar }).FirstOrDefault();
                if (k != null)
                {
                    DodajKomentar(k.Id, komentator.Ime + " " + komentator.Prezime, komentar, 0, 0);
                }
            }
        }
        ...
        private void cbxListOcijena_Validating(object sender, CancelEventArgs e)
        {
            if (cbxListOcijena.SelectedIndex == 0)
            {
                e.Cancel = true;
                errorProvider.SetError(cbxListOcijena, Messages.Text_Length_3_200);
            }
        }
        ...
        private void cbxGrad_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                e.Cancel = true;
                errorProvider.SetError(cbxGrad, Messages.CBX_Grad);
            }
        }

        private void cbxKucaGrad_Validating(object sender, CancelEventArgs e)
        {
            if (cbxKucaGrad.SelectedIndex == 0)
            {
                e.Cancel = true;
                errorProvider.SetError(cbxKucaGrad, Messages.CBX_Grad);
            }
        }
        ...
        private void cbxDvoranaProjekcije_Validating(object sender, CancelEventArgs e)
        {
            if (cbxDvoranaProjekcije.SelectedIndex == 0)
            {
                e.Cancel = true;
                errorProvider.SetError(cbxDvoranaProjekcije, Messages.CBX_Dvorana);
            }
        }
